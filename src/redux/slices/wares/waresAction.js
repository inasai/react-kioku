...
export const WaresActionCreators = {
	fetchDevices:
		(currentPage, category, sortBy, order, search) => async (dispatch) => {
			try {
				const wares = await waresApi.getWares(
					currentPage,
					category,
					sortBy,
					order,
	...
	addDevice:
		(image, title, types, price, category, rating) => async (dispatch) => {
			try {
				const ware = await waresApi.addWare(
					image,
					title,
					types,
					price,
